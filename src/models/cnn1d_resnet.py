...
Why residual connections over plain CNN1D?
  ...
  add more layers without degradation — ResNets get accuracy gains "for free"
  from depth that plain CNNs cannot exploit.

Architecture:
  1. Input projection  : Conv1d(248 -> 64, k=1) — learned spatial mixing
  2. Residual blocks   : 3 x ResBlock(64->128, 128->256, 256->256)
...
class CNN1DResNet(nn.Module):
    ...
    def __init__(self):
        ...
        # 1. Spatial projection: mix 248 sensors into the first channel count
