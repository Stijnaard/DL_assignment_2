...
class CustomDataset(Dataset):
    ...
    def __init__(self, segments: Sequence[DataSegment], pipeline: Optional[Pipeline] = None, device: Optional[str] = None) -> None:
        ...
        self.feature_tensor: Tensor = tensor(concatenate(matrices, axis=0)).to(device)

        # 4. construct the label vector:
        sorted_tasks = sorted(TASK_TYPES)
        self.label_vector: Tensor = tensor([sorted_tasks.index(segment.get_task()) for segment in segments]).to(device)
        return None
