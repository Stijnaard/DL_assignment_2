...
Classify brain states (Rest / Motor / Math / Memory) from MEG signals using deep learning models.
...
|   NOTE: each folder contains rest, task_motor, task_story_math, task_working_memory files (all .h5). Withing each folder every type has as many files per type. But between files these are different, for example Intra/train does not have as many files as Intra/test.
Intra has 68  windows with this setup per type.
Cross has 408 windows with this setup per type.
│   ├── Intra/
...
└── src/
    ...
    ├── models/
    |   ├── lstm.py          ← Bidirectional Stacked LSTM
...
|-------|-------------|----------|
| `LSTM`| bidirectional RNN | Baseline comparison |
...
| `cnn_transformer` | CNN frontend + Transformer encoder |
---
